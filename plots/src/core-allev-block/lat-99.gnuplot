...
set ylabel "99th Percentile Latency (\\unit{\\micro\\second})"
...
plot for [i=0:2] Files[1] u 4:(1.0e-3 * $6) every ::(start_pt(i, 1))::(start_pt(i, 1) + CoreCnts[1] - 1) with linespoints title myTitle(i, 1) ls LineStyles[i+1] dt DashStyles[1], \
	for [i=0:2] Files[2] u 4:(1.0e-3 * $6) every ::(start_pt(i, 2))::(start_pt(i, 2) + CoreCnts[2] - 1) with linespoints title myTitle(i, 2) ls LineStyles[i+1] dt DashStyles[2]