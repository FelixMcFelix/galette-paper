...
plot for [i=0:1] file(i) u 1:6 every StridePs[1]::(1 + StrideRates[1] * RateIdxXdp[i + 1]) with linespoints title myTitle(i) ls LineStyles[i+1] dt DashStyles[1], \
	for [i=0:1] file(2+i) u 1:6 every StridePs[2]::(1 + StrideRates[2] * RateIdxUpcall[i + 1]) with linespoints title myTitle(2+i) ls LineStyles[i+1] dt DashStyles[2]